...
export async function testInvidiousInstance(base: string): Promise<{ version: string }> {
  const res = await fetch(`${base}/api/v1/stats`, { signal: timeoutSignal(8000) });
...
export async function invidiousChannelInfo(
  ...
  const res = await fetch(`${base}/api/v1/channels/${encodeURIComponent(channelId)}`, { signal });
...
export async function invidiousResolveHandle(
  ...
  const res = await fetch(`${base}/api/v1/search?q=${encodeURIComponent(clean)}&type=channel`, {
    signal,
  });
  if (!res.ok) throw new Error(`Invidious HTTP ${res.status}`);
...
export async function invidiousChannelLatest(
  ...
  const res = await fetch(`${base}/api/v1/channels/${encodeURIComponent(channelId)}/latest`, {
    signal,
  });
  if (!res.ok) throw new Error(`Invidious HTTP ${res.status}`);
