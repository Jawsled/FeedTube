...
// In the local web app there is no extension context: cross-origin requests
// go through the same-origin relay server (/proxy), which sets the per-host
// Origin/Referer/Accept headers that SoundCloud's DataDome protection expects.
async function scFetch(type: 'text' | 'json', url: string): Promise<{ ok: boolean; status: number; text: string }> {
  try {
    const init: RequestInit = type === 'json' ? { headers: { Accept: 'application/json' } } : {};
...
async function fetchText(url: string, signal?: AbortSignal): Promise<string> {
  const res = await scFetch('text', url);
...
async function fetchJson<T>(url: string, signal?: AbortSignal): Promise<T> {
  const res = await scFetch('json', url);
