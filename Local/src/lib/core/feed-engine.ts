...
async function refreshOne(
  ...
  const limitedVideos = outcome.videos.length > settings.feedFetchLimit
    ? outcome.videos.slice(0, settings.feedFetchLimit)
    : outcome.videos;
  const newVideos: NewVideo[] = limitedVideos.map((v) => ({
    id: v.id,
    source,
    channelId: ch.id,
    title: v.title,
  ...
  log('info', `✓ Got ${limitedVideos.length} video${limitedVideos.length === 1 ? '' : 's'} via ${backendUsed ?? 'unknown'}${outcome.detail ? ` (${outcome.detail})` : ''} (added ${added} new)`);
