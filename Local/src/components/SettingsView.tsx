...
import { probeBilibili, probeOdysee, probePeerTube, probeSoundCloud } from '../lib/core/probes';
...
function dispatchProbe(probe: ProbeName): Promise<string> {
  const label = PROBE_LABELS[probe].replace('Test ', '');
  const fn =
    probe === 'odysee' ? probeOdysee :
    probe === 'bilibili' ? probeBilibili :
    probe === 'peertube' ? probePeerTube :
    probeSoundCloud;
  return fn()
    .then((res) => (res.ok ? `✓ ${res.message}` : `✗ ${res.message}`))
    .catch((e: unknown) =>
      `✗ ${label} API unreachable: ${e instanceof Error ? e.message : String(e)}`,
    );
}
...
export function SettingsView() {
  ...
  const applyInvidious = async () => {
    ...
    await update({ invidiousInstance: base });    setTestResult(null);
  ...
  const runProbe = async (source: SourceKind) => {
    ...
    const result = await dispatchProbe(probe);
  ...
  return (
    <div style={{ maxWidth: 620 }}>
      ...
      <div class="card" style={{ padding: '4px 18px' }}>
        <Row label="Background refresh" hint="How often to check channels for new uploads while FeedTube is open">
          <select
      ...
      <div class="card" style={{ padding: '4px 18px' }}>
        <Row
          label="Auto-refresh on app start"
          hint="Trigger a feed refresh when FeedTube loads"
        >
          <Toggle
      ...
      <div class="card" style={{ padding: '4px 18px' }}>
        ...
        <div style={{ padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
          ...
          <div class="faint" style={{ marginTop: 8 }}>
            Requests to this instance are routed through the local relay server. Public instance list:
