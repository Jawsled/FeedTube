...
import { cancelRefresh as engineCancelRefresh, clearStoredLog, readStoredLog, readStoredStatus, refreshAll } from '../lib/core/feed-engine';
...
export function FeedView({ onGoToSubs }: { onGoToSubs: () => void }) {
  ...
  const refreshChannel = () => {
    ...
    void refreshAll({ channelIds: [channelFilter], force: true });
  };
  ...
  const startRefresh = () => {
    void refreshAll(tagFilter ? { tags: [tagFilter], force: true } : { force: true });
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (tagFilter) {
      const channelIdsWithTag = new Set(
        Array.from(d.channelById.entries())
